...
public class CFX_Demo_New : MonoBehaviour
{
	...
	void Update()
	{
		if (InputSystem.GetDevice<Keyboard>().leftArrowKey.wasPressedThisFrame)
		{
			prevParticle();
		}
			else if (InputSystem.GetDevice<Keyboard>().rightArrowKey.wasPressedThisFrame)
		{
			nextParticle();
		}
		else if(InputSystem.GetDevice<Keyboard>().deleteKey.wasPressedThisFrame)
		{
			destroyParticles();
		}
		...
		// float scroll2 = InputSystem.GetDevice<Mouse>().scroll.;
		// if(scroll != 0f)
		// {
